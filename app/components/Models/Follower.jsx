...
const Follower = () => {
  const router = useRouter();
    ...
    return (
        <>
          <div >
            <dialog id="my_modal_2" className="modal w-3/4">
              <div className="modal-box bg-stone-900 w-full p-5">
              {followerss && followerss.length === 0 ? (
                <div className='flex justify-center items-center h-screen text-white text-opacity-25'>
                  <p>No followers</p>
                </div>
              ) : (
                followerss.map((user, index) => (
                  <div key={index} className="w-full h-auto flex flex-col md:flex-row justify-between items-center border-b-[1px] border-white border-opacity-20 text-white mt-3 p-3">
